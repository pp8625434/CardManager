...
if(_c0.id=="yy_tree_node_id_temp"){
_c0.id="";
_14f(_b9,_c0);
}
...
$.fn.validatebox.defaults={required:false,validType:null,delay:200,missingMessage:"该输入项为必输项.",invalidMessage:null,tipPosition:"right",rules:{email:{validator:function(_3c0){
...
},message:"请输入有效的电子邮件地址."},url:{validator:function(_3c1){
...
idcard : { validator : function(value) {  return /^(^\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/i.test(value); },message : "身份证号码格式不正确" },
minLength: {validator: function(value, param){return value.length >= param[0];},message: "请输入至少{0}个字符."},
maxLength: {validator: function(value, param){return value.length <= param[0];},message: "请输入最多{0}个字符."},
phone : {validator : function(value) { return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value); }, message : "格式不正确,请使用下面格式:020-88888888"}, mobile : {validator : function(value) { return /^(13|15|18)\d{9}$/i.test(value); }, message : "手机号码格式不正确"}, 
intOrFloat : {validator : function(value) {  return /^\d+(\.\d+)?$/i.test(value); }, message : "请输入数字，并确保格式正确" }, currency : { validator : function(value) {return /^\d+(\.\d+)?$/i.test(value); }, message : "货币格式不正确" }, qq : {validator : function(value) { return /^[1-9]\d{4,9}$/i.test(value); }, message : "QQ号码格式不正确" }, integer : {validator : function(value) { return /^[+]?[1-9]+\d*$/i.test(value); }, message : "请输入整数"}, age : {validator : function(value) { 
 return /^(?:[1-9][0-9]?|1[01][0-9]|120)$/i.test(value); }, message : "年龄必须是0到120之间的整数"}, chinese : {validator : function(value) { return /^[\Α-\￥]+$/i.test(value); }, message : "请输入中文" }, english : {validator : function(value) { return /^[A-Za-z]+$/i.test(value); },  message : "请输入英文"  }, unnormal : {validator : function(value) {return /.+/i.test(value); }, message : '输入值不能为空和包含其他非法字符' }, username : { validator : function(value) {return /^[a-zA-Z][a-zA-Z0-9_]{5,15}$/i.test(value);  }, message : '用户名不合法（字母开头，允许6-16字节，允许字母数字下划线）' },faxno : {validator : function(value) { 
return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value); }, message : '传真号码不正确' }, zip : {validator : function(value) {return /^[1-9]\d{5}$/i.test(value); }, message : '邮政编码格式不正确'}, ip : {validator : function(value) { return /d+.d+.d+.d+/i.test(value);}, message : 'IP地址格式不正确'  },  name : { validator : function(value) { return /^[\Α-\￥]+$/i.test(value)|/^\w+[\w\s]+\w+$/i.test(value); }, message : '请输入姓名,可以是中文或英文' },date : {validator : function(value) { //格式yyyy-MM-dd或yyyy-M-d
 return /^(?:(?!0000)[0-9]{4}([-]?)(?:(?:0?[1-9]|1[0-2])\1(?:0?[1-9]|1[0-9]|2[0-8])|(?:0?[13-9]|1[0-2])\1(?:29|30)|(?:0?[13578]|1[02])\1(?:31))|(?:[0-9]{2}(?:0[48]|[2468][048]|[13579][26])|(?:0[48]|[2468][048]|[13579][26])00)([-]?)0?2\2(?:29))$/i.test(value); },message : '清输入合适的日期格式'}, same:{  validator : function(value, param){ 
  if($("#"+param[0]).val() != "" && value != ""){ return $("#"+param[0]).val() == value; }else{ return true; }  }, message : '两次输入的内容不一致！' }, 
length:{validator:function(_3c2,_3c3){
var len=$.trim(_3c2).length;
return len>=_3c3[0]&&len<=_3c3[1];
},message:"输入内容长度必须介于{0}和{1}之间."},remote:{validator:function(_3c4,_3c5){
var data={};
data[_3c5[1]]=_3c4;
...
var _3cb="yy_frame_"+(new Date().getTime());
