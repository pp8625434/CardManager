...
//初始化区域图的对象
	...
	if(property.headLabel){
      tmp+="<label title='"+(property.initLabelText||"newFlow_1")+"' "
        //+(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"newFlow_1")+"</label>";
      	+">流程图名称："+(property.initLabelText||"newFlow_1")+"</label>";
		}
	...
	//if(this.$editable){
		 ...
		 this.$workArea.on("click",{inthis:this},function(e){
				...
				//update by foo
				This.addNode(This.$id+"_node_"+This.$max,{nId:This.$id+"_node_"+This.$max,name:"node_"+This.$max,left:X,top:Y,type:This.$nowType});
				This.$max++;
			  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
	...
	addNode:function(id,json){
		//update by foo
		if(this.$nodeDataN!=null){
			this.$nodeDataN +=","+id;
		}else{
			this.$nodeDataN = id;
		}
		
		//alert(this.$nodeDataN);
	...
	initWorkForNode:function(){
        ...
        this.$workArea.delegate(".GooFlow_item", "mousedown", { inthis: this }, function (e) {
            if (e.button == 2) {
               ...
               // alert(JSON.stringify(This.$nodeData));
                if (This.$nowType == "cursor") {
                	
                    //右键的是节点对象
                    //alert("右键");
                    var menuBindDiv = $("#div_flowNodeRightMenu")[0];
                    if (menuBindDiv == undefined) {

                        //不存在，创建菜单
                        menuBindDiv = document.createElement("div");
                        menuBindDiv.id = "div_flowNodeRightMenu";
                        menuBindDiv.className = "smart_menu_box";
                        document.body.appendChild(menuBindDiv);

                        //菜单主体
                        var menuBody = document.createElement("div");
                        menuBody.id = "div_menuBody";
                        menuBody.className = "smart_menu_body";
                        menuBindDiv.appendChild(menuBody);

                        //菜单分段
                        var menuSection = document.createElement("ul");
                        menuSection.id = "ul_section_1";
                        menuSection.className = "smart_menu_ul";
                        menuBody.appendChild(menuSection);

                        //菜单分段明细
                        var menuItem1 = document.createElement("li");
                        menuItem1.id = "li_menuItem_1";
                        menuItem1.className = "smart_menu_li";

                        var elementA = document.createElement("a");
                        elementA.className = "smart_menu_a";
                        elementA.href = "javascript:";
                        elementA.innerHTML = "权限设置";
                        menuItem1.appendChild(elementA);

                        //菜单分段明细
                        var menuItem2 = document.createElement("li");
                        menuItem2.id = "li_menuItem_1";
                        menuItem2.className = "smart_menu_li";

                        var elementA2 = document.createElement("a");
                        elementA2.className = "smart_menu_a";
                        elementA2.href = "javascript:";
                        elementA2.innerHTML = "人员管理";
                        menuItem2.appendChild(elementA2);


                        //菜单分段分隔符
                        var menuItem3 = document.createElement("li");
                        menuItem3.id = "li_menuItem_1";
                        menuItem3.className = "smart_menu_li_separate";
                        menuItem3.innerHTML = "&nbsp;";


                        //菜单分段明细
                        var menuItemEnd = document.createElement("li");
                        menuItemEnd.id = "li_menuItem_1";
                        menuItemEnd.className = "smart_menu_li";

                        var elementEnd = document.createElement("a");
                        elementEnd.className = "smart_menu_a";
                        elementEnd.href = "javascript:";
                        elementEnd.onclick = function (e) {
                            $("#div_flowNodeRightMenu").hide();
                            
                        };
                        elementEnd.innerHTML = "关闭";
                        menuItemEnd.appendChild(elementEnd);

                        menuSection.appendChild(menuItem1);
                        menuSection.appendChild(menuItem2);
                        menuSection.appendChild(menuItem3);
                        menuSection.appendChild(menuItemEnd);
                    }
                    var jqueryMenu = $("#div_flowNodeRightMenu");
                    if (jqueryMenu) {
                        jqueryMenu.css({
                            display: "block",
                            left: e.clientX,
                   ...
                   // $("#result").val((jqueryMenu.html()));
                    // $("#div_flowNodeRightMenu2").show();
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
				...
				//update by foo
				This.addLine(This.$id+"_line_"+This.$max,{nId:This.$id+"_line_"+This.$max,from:lineStart.id,to:this.id,name:""});
				This.$max++;
			}
			else{
				if(lineStart){
	...
	//当前导入的josn 格式无法直接解析为对象，重写
	exportJsonData:function(type){
		var ids;
		var jsonLine="";
		var jsonNode="";
		if(type==null) return;
		if(type=="node"){
			if(this.$nodeDataN != null){
				ids = this.$nodeDataN.split(",");
				for(var i=0;i<ids.length;i++){
					var temp = this.getItemInfo(ids[i],"node");
					if(temp!=null){
						if(jsonNode.length>0){
							jsonNode +=",";
							jsonNode +='{"nId":"'+ids[i]+'","type":"'+temp.type+'","left":'+temp.left+',"top":'+temp.top;
							jsonNode +=',"alt":'+temp.alt+',"name":"'+temp.name+'"';
							if(temp.width != undefined){
								jsonNode +=',"width":'+temp.width+',"height":'+temp.height;
							}
							jsonNode +="}";
						}else{
							jsonNode ='{"nId":"'+ids[i]+'","type":"'+temp.type+'","from":"'+temp.from+'","to":"'+temp.to;
							jsonNode +='","alt":'+temp.alt+',"name":"'+temp.name+'"';
							if(temp.M != undefined){
								jsonNode +=',"M":'+temp.M;
							}
			...
			if(this.$lineDataN != null){
				ids = this.$lineDataN.split(",");
				for(var i=0;i<ids.length;i++){
					var temp = this.getItemInfo(ids[i],"line");
					if(temp!=null){
						if(jsonLine.length>0){
							jsonLine +=",";
							jsonLine +='{"nId":"'+ids[i]+'","type":"'+temp.type+'","from":"'+temp.from+'","to":"'+temp.to;
							jsonLine +='","alt":'+temp.alt+',"name":"'+temp.name+'"';
							if(temp.M != undefined){
								jsonLine +=',"M":'+temp.M;
							}
							jsonLine +="}";
...
///////////以下为有关画线的方法
	...
	addLine:function(id,json){
		...
		//update by foo 
		//var temp ='{"nId":'+id+',"type":'+this.$lineData[id].type+',"from":'+this.$lineData[id].from+',"to":'+this.$lineData[id].to;
		//temp +=',"alt":'+this.$lineData[id].alt+',"name":'+this.$lineData[id].name;
		//if(this.$lineData[id].M != undefined){
		//	temp +=',"M":'+this.$lineData[id].M;
		//}
		//temp+='}';
		if(this.$lineDataN!=null){
			this.$lineDataN +=","+id
			//this.$lineDataN +=","+JSON.stringify(this.$lineData[id]);
		}else{
			this.$lineDataN = id;
		}
		//alert(JSON.stringify(this.$lineData[id]));
		//alert(this.$lineDataN);
		++this.$lineCount;
		if(this.$editable){
			this.$lineData[id].alt=true;
			if(this.$deletedItem[id])	delete this.$deletedItem[id];//在回退删除操作时,去掉该元素的删除记录
